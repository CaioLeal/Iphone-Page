<div align="center">
  
</div>

# 📱 iPhone 17 Pro - Landing Page

**`React`** **`Tailwind CSS`** **`Vite`**

This project is a high-level landing page inspired by the Apple ecosystem, focusing on the presentation of the **iPhone 17 Pro**. The main goal was to apply advanced utility styling concepts and efficient componentization to create a premium user experience.

---

### 📍 Why this Project?

This project was developed with a focus on **visual fidelity** and **performance**. It recreates the Apple browsing experience, using an elegant dark mode interface, fluid animations, and modern interactivity.

The main technical highlights include:
* **React Componentization:** Logical division between `Hero`, `Highlights`, `Colors`, and `Intro` for better scalability.
* **Advanced Tailwind CSS:** Use of utility classes to ensure full responsiveness, `backdrop-blur` effects, and smooth transitions.
* **State Management:** Implementation of dynamic color and model switching for the iPhone using the `useState` hook.

---

### 🌐 View the project:
[View Project Online](https://caioleal.github.io/Iphone-Page/)

---

### 🔗 Project Link:
[Repository: Iphone-Page](https://github.com/CaioLeal/Iphone-Page)

---

### ⭐ How to use this project locally?

To run this project on your machine, you will need **Node.js** installed.

1. **Clone this repository:**
```bash
git clone [https://github.com/CaioLeal/Iphone-Page.git](https://github.com/CaioLeal/Iphone-Page.git)
```
2. **Navigate to the project folder:**
```bash
cd Iphone-Page
```
3. **Install dependencies:**
```bash
npm install
```
4. **Install dependencies:**
```bash
npm run dev
```

---

[!IMPORTANT]
🚀 Learning Journey: This project was developed during a practical class with mentor Rodolfo Mori, exploring market best practices for modern layouts and productivity with Tailwind CSS.

### 🛠 Technologies Used:

- [React](https://react.dev/)
- [Tailwind CSS](https://tailwindcss.com/)
- [Vite](https://vite.dev/)
- [Lucide Icons](https://lucide.dev/)